...
**验证基线**:Claude Code CLI `2.1.168`(2026-06-08 容器内 egress 审计 + 公网链路实测)、`2.1.170`(2026-06-09 §2 端点提取:二进制硬编码域名 + CONNECT 抓包;与 `ci/egress/known-hosts.txt` 漂移守卫基线同版)、模型 `claude-opus-4-8`。后续版本行为可能变,以实测为准。
...
- **TLS 指纹(JA3/JA4)、HTTP/2 指纹**:由 CC 的网络栈(Node.js / undici)决定,**不是中转能选择的**。跑真 CC → 指纹落在「真 CC」的分布内。注意它随 Node 版本变(本机 v24、审计容器 v20 即不同),所以"同出口普通登录设备"本身也不是单一指纹,而是一族真 CC 指纹;中转设备只要也跑真 CC,就在同一族内。**前提是中转不重构请求、内层 TLS 由真 CC 自己端到端发起**——此前提对**纯透传 host**(盲隧道、端到端 TLS)成立,对 **MITM host(`api.anthropic.com`/`console.anthropic.com`)不成立**:cc-mysub 终结内层 TLS 后由 Go 栈重新发起对上游的 TLS,故该端点的 transport 指纹是 Go 栈的而非 undici 的(详见 §3 与 `../SECURITY.md`「诚实降级」)。
...
- **网络(统一出口 + 指纹)**:所有中转设备经同一出口出网 = 满足「同出口」基线。指纹等效**仅对纯透传 host 成立**:datadog/downloads 等盲隧道是**端到端 TLS**(claude 与真上游直接做 TLS,中转只搬 TCP 字节),到达官方的 TLS/HTTP2 指纹仍是真 CC 的。**MITM host(`api.anthropic.com`/`console.anthropic.com`)不同**:中转在出口**终结** claude 的内层 TLS、再由 cc-mysub 的 **Go 网络栈对上游重新发起** TLS(ClientHello/JA3 是 Go `crypto/tls` 的、出站默认协商 HTTP/2 并经 h1→h2 重序列化),故这些端点上到达官方的 transport 指纹是 **cc-mysub(Go) 的、非 undici 的**——这是「MITM 换 token」的固有代价(盲隧道无法换 token)。中转**不重建请求、不替 CC 说话**:请求的**应用语义层(头值/body/query/cch)逐字节保真透传**,只是承载它的 TLS/HTTP2 transport 帧在 MITM host 上由 Go 栈重序列化(诚实边界见 `../SECURITY.md`「诚实降级:不可区分性」)。
...
**一句话**:跑官方 CC 二进制 + 只在「凭据值」和「统一出口」两处做最小处理 + 绝不重构请求或注入痕迹,官方侧收到的就是一组带真 token、真 CC 应用层请求(头/body/cch)的请求,来自一个统一出口(`api.anthropic.com` 的外联 transport 指纹因 MITM 换 token 为 cc-mysub 的 Go 栈,见 §3)——与「一个订阅所有者在该出口后多设备正常登录」在数据层面等效。
